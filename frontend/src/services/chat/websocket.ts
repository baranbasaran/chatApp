...
export class WebSocketService {
  ...
  connect(token: string) {
    console.log(
      "[WebSocket] Attempting to connect with token:",
      token ? "present" : "missing"
    );
    console.log(
      "[WebSocket] Current socket state:",
      this.socket ? "exists" : "null"
    );

    if (this.socket?.connected) {
      console.log(
        "[WebSocket] Socket already connected, reusing existing connection"
      );
      return this.socket;
    }
    ...
    if (this.socket) {
      console.log(
        "[WebSocket] Socket exists but not connected, cleaning up first"
      );
    ...
    this.socket.on("reconnect_attempt", (attemptNumber: number) => {
      console.log(
        "[WebSocket] Attempting to reconnect, attempt #",
        attemptNumber
      );
    });
  ...
  disconnect() {
    console.log(
      "[WebSocket] Disconnect called, socket state:",
      this.socket ? "exists" : "null"
    );
    if (this.socket) {
